...
class PatchHandler3D():
    ...
    def load_patches_from_index_file(self, indexes):
        ...
        u_patch, u_hr_patch, v_patch, v_hr_patch, w_patch, w_hr_patch, venc, mask_patch = self.load_vectorfield(hd5path, lr_hd5path, idx, mask_index, patch_index, hr_patch_index)
    ...
    def load_vectorfield(self, hd5path, lr_hd5path, idx, mask_index, patch_index, hr_patch_index):
        ...
        # U-LR, HR, MAG, V-LR, HR, MAG, w-LR, HR, MAG, venc, MASK
        return lowres_images[0].astype('float32'), hires_images[0].astype('float32'), \
            lowres_images[1].astype('float32'), hires_images[1].astype('float32'), \
            lowres_images[2].astype('float32'), hires_images[2].astype('float32'), \
            np.float32(global_venc), mask.astype('float32')
...
class PatchHandler3D_2():
    ...
    def __init__(self, data_dir, patch_size, res_increase, batch_size, mask_threshold=0.6):
        ...
        self.venc_colnames = ['high_venc','high_venc','high_venc']
        #self.venc_colnames = ['u_max','v_max','w_max']
        
        #self.mag_colnames  = ['mag_u','mag_v','mag_w']
        self.mask_colname  = 'tof_mask'
